...
OTHER_CHANNELS = os.environ.get("OTHER_CHANNELS", "").split(",")  # e.g., "https://t.me/ch2,https://t.me/ch3"

# Movie File IDs (ခင်ဗျား Bot ဆီ upload ပြီးရရှိတဲ့ ID ၃ခု)
MOVIE_FILE_IDS = [
    "FILE_ID_1",
    "FILE_ID_2",
    "FILE_ID_3"
...
# Admin User IDs (ခင်ဗျား Telegram User ID ထည့်ပါ)
ADMIN_IDS = [int(os.environ.get("ADMIN_ID", "0"))]  # ဥပမာ 123456789

# Database (simple JSON file for stats, schedules)
...
# ---------- User Side: Callback for Movie Button ----------
async def movie_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if maintenance_mode:
        await query.edit_message_text("⚠️ Bot သည် ပြုပြင်ထိန်းသိမ်းမှုမုဒ်တွင် ရှိပါသည်။ နောက်မှ ထပ်ကြိုးစားပါ။")
        return
...
async def schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    await update.message.reply_text("⏳ /schedule အတွက် အသေးစိတ် ထည့်သွင်းရန် လိုအပ်ပါသည်။ (ရိုးရိုးဥပမာသာ)")

async def listschedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    data = load_data()
    if not data["schedules"]:
        await update.message.reply_text("စာရင်းမရှိပါ။")
    else:
        text = "📋 **Schedules:**\n"
        for s in data["schedules"]:
            text += f"ID: {s['id']} | {s['time']} | {s['message']}\n"
        await update.message.reply_text(text)

async def cancelschedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    # usage: /cancelschedule 3
    await update.message.reply_text("Schedule ဖျက်ရန် ID ထည့်ပါ (demo)")
...
async def delete_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    # /delete file_id
    await update.message.reply_text("/delete command က file ID ဖျက်ရန် (ထပ်ရေးရန် လိုအပ်)")

async def deleteall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    # reset all movies?
    await update.message.reply_text("အားလုံးဖျက်ရန် အတည်ပြုရန် ထပ်မံအတည်ပြုမှု လိုအပ်ပါသည်။")
...
async def mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔇 Maintenance mode ဖွင့်ထားပါသည်။ User များ movie မရနိုင်ပါ။")
