...
async def cancelschedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    await update.message.reply_text("❌ Schedule cancel - demo")

async def delete_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    await update.message.reply_text("/delete command - demo")
...
# ---------- Bot Polling Loop ----------
def run_bot():
    """Bot ကို polling mode နဲ့ run မယ်၊ crash ရင် ပြန်စမယ်"""
    while True:
        try:
